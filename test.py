...
def run():
    ...
    db = connect['allsite']
    collection = db['stock']
    find_brand = 'Montecristo'
    find_cigar_name = '1935'
    find_stock = 'sold out'
    find_list = collection.find({"$and": [{"Brand": {"$regex":find_brand,'$options':'i'}}, {"cigar_name": {"$regex":find_cigar_name,'$options':'i'}},{"stock":{"$not": {"$regex":find_stock,'$options':'i'}}}]})
    for i in find_list:
        tmp_items = i['itemurl']
